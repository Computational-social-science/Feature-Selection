...
def compinfo(x, y, z=None, s=None):
    ...
    return info
def FSMB(data: pd.DataFrame, alpha=0.01) -> Set[int]:
...
datasets = [ 'Arcene',]#'Dermatology',
# datasets = [ 'Authorship','CLL_SUB_111','Colon','Dermatology','dexter','dna', 'Factors','GLIOMA','Isolet','madelon','Movement_libras','Musk1','ORL','Orlraws10P','Pixel',  'Prostate_GE', 'Sorlie', 'Su','SRBCT','spambase','splice','Synthetic_control', 'TOX_171', 'Waveform','Wdbc',  'WarpPIE10P','WarpAR10P', 'Yeoh', 'Yale', ]#
# datasets = ['Movement_libras']
base_url = r"./dataset/"
count = pd.DataFrame(index=range(len(datasets)), columns=range(len([FSMB])))
count.columns = ["FSMB"]
...
for dataname in datasets:
    ...
    fsmb = FSMB(data_processed, alpha=0.05)
    result = fsmb
    # print(result)

    # fresult = np.array(list(result))
    count["FSMB"][dataname] = int(ifdep.call_count)
    print(count)
    count.to_excel('tables/' + 'fsmbCIcount.xlsx', index=True)
    # print()
    # print("马尔可夫毯计算结果:")
