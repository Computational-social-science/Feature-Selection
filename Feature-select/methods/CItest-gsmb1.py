...
class GSMB:
    ...
    def fit(self) -> dict:
        ...
        :return: 字典 {变量: MB列表}
        """
        return {var: self.compute_markov_blanket(var) for var in self.variables[-1:]},self.time
def compare_compute_dep(old_compute_dep, new_compute_dep, data, n_states, n_cases, num_tests=100, tol=1e-6):
    ...
    return diffs
#'Isolet','EAMB',EAMB,'FBED','LRH','IAMB','Inter_IAMB','Fast_IAMB'
methodsname = ['GSMB']#,,,,'FBED','LRH''IAMB','Inter_IAMB','Fast_IAMB'
methods = [GSMB]#,,,,FBEDLRHIAMB,Inter_IAMB,Fast_IAMBFBED,LRH,IAMB,Inter_IAMB,Fast_IAMB
...
datasets = [ 'Colon','dexter','dna', 'Factors','GLIOMA','Isolet','madelon','Movement_libras','Musk1','ORL','Orlraws10P','Pixel',  'Prostate_GE', 'Sorlie', 'Su','SRBCT','spambase','splice','Synthetic_control', 'TOX_171', 'Waveform','Wdbc',  'WarpPIE10P','WarpAR10P', 'Yeoh', 'Yale','Arcene', ]#
base_url = r"./dataset/"
import numpy as np
...
for dataname in datasets:
    ...
    gsmb = GSMB(data_processed, alpha=0.05)
    for method in methods:
        ...
        print(count)
        count.to_excel('tables/' + '1gsmbCIcount.xlsx', index=True)
    ...
    # for fold, (train_index, test_index) in enumerate(kfold.split(data_processed)):  # fold赋值之后就是0-9
        ...
        # if fold == 0:
            ...
            #     ifdep.call_count = 0
            #     result = EAMB(str(X.shape[1]), data_train, 0.1)
