...
def resulttest(result, data_train, data_test):
    ...
    bacc = roc_auc_score(data_test.iloc[:, -1], bpredict_labels)
    kacc = roc_auc_score(data_test.iloc[:, -1], kpredict_labels)
    sacc = roc_auc_score(data_test.iloc[:, -1], spredict_labels)
    racc = roc_auc_score(data_test.iloc[:, -1], rpredict_labels)  # 种类个数
    ...
    for method in methods:
        for fold, (train_index, test_index) in enumerate(kfold.split(data_processed)):  # fold赋值之后就是0-9
            ...
            if result.size == 0:
                ...
                f1,acc = resulttest(result, data_train, data_test)
                allf1b.append(f1['b'])
                allf1k.append(f1['k'])
                allf1s.append(f1['s'])
                allf1r.append(f1['r'])
...
bf1table.to_excel('tables/' + 'realbf1table.xlsx', index=True)
kf1table.to_excel('tables/' + 'realkf1table.xlsx', index=True)
sf1table.to_excel('tables/' + 'realsf1table.xlsx', index=True)
rf1table.to_excel('tables/' + 'realrf1table.xlsx', index=True)
...
bstdtable.to_excel('tables/' + '1esmbsp-bastdctable.xlsx', index=True)
kstdtable.to_excel('tables/' + '1esmbsp-kastdctable.xlsx', index=True)
sstdtable.to_excel('tables/' + '1esmbsp-sastdctable.xlsx', index=True)
rstdtable.to_excel('tables/' + '1esmbsp-rastdctable.xlsx', index=True)