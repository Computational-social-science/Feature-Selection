...
for dataname in datasets:
    ...
    for fold, index, in enumerate(kfold.split(data_processed)):  # fold赋值之后就是0-9
        print("\n当前处理到第", fold, "折")
        # data_train, data_test = data_processed.iloc[index], data_processed.iloc[index]  # 提取训练集条目
        result = IAMB(data_processed)
        print("result:",result,)
        np_result = np.array(result, dtype=object)  # 使用 dtype=object 以允许不规则的长度
        # 保存到txt文件
        file_name = r"feature_select_result/" + 'New/IAMB' + '/' + dataname  + "_result-"+str(fold)+".txt"
