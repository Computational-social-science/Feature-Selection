...
def csmdccmrtest(time, result, X_test, y_test, X_train, y_train):
    ...
    for i in range(time):
        ...
        for item in nowpick:
            ...
            #每个分类器在当前情况下的分类概率
            b10auc.append(bprobabilities)#记录n种分类器单次概率
    ...
    predict_pro   = {'b': all_bpredict_probabity, 'k': all_kpredict_probabity, 's': all_spredict_probabity, 'r': all_rpredict_probabity,}
    predict_lable = {'b': all_bpredict_labels, 'k': all_kpredict_labels, 's': all_spredict_labels, 'r': all_rpredict_labels,}
