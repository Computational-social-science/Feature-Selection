...
def csmdccmrtest(time, result, X_test, y_test, X_train, y_train):
    ...
    for item in nowpick:
        ...
        k10auc.append(kprobabilities)#记录n种分类器单次分类概率
        s10auc.append(sprobabilities)
        r10auc.append(rprobabilities)
    bpredicted_labels = (np.argmax(bresultmax, axis=1))#根据分类概率求分类标签
    ...
    spredict_probabity.append(s10auc)#记录time分类概率
    ...
    file_name1 = r"result/" + method +'/'+'2_'+ dataname + str(fold) +  "_predict_pro.mat"
    file_name2 = r"result/" + method +'/'+'2_'+ dataname + str(fold) +  "_predict_lable.mat"
