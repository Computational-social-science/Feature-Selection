...
datanames = ['Waveform','Wdbc', 'Synthetic_control','Authorship', 'Factors', 'Pixel', 'Isolet', 'ORL', 'WarpPIE10P', 'Su',
            'Movement_libras','Prostate_GE', 'TOX_171', 'Orlraws10P', 'CLL_SUB_111', 'Yeoh','Musk1', 'Sorlie', 'Yale', 'WarpAR10P', 'GLIOMA', 'Arcene', ]#'Dermatology',
base_url = r"./dataset/"
...
def csmdccmrtest(time, result, X_test, y_test, X_train, y_train):
    ...
    file_name = r"accuracy/2/" + dataname + '_' + method + "_accuracy.mat"
...
for dataname in datanames:
    ...
    for n in methods1:
        ...
        for fold, (train_index, test_index) in enumerate(kfold.split(X, y)):  # fold赋值之后就是0-9
            ...
            # print(result)
            # 对每一个数量的特征选择做测试
            bnacc,knacc,rnacc,snacc = csmdccmrtest(int(time), result, X_test, y_test, X_train, y_train)
