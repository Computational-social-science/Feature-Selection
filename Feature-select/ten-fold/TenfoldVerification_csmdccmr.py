...
def csmdccmrtest(time, result, X_test, y_test, X_train, y_train):
    for i in range(time):
        ...
        for item in nowpick:
            ...
            b0auc = roc_auc_score(y_test[0],bprobabilities,multi_class='ovo',average='macro',)
            k0auc = roc_auc_score(y_test[0],kprobabilities,multi_class='ovo',average='macro',)
            s0auc = roc_auc_score(y_test[0],sprobabilities,multi_class='ovo',average='macro',)
            r0auc = roc_auc_score(y_test[0],rprobabilities,multi_class='ovo',average='macro',)
            b10auc.append(b0auc)
            k10auc.append(k0auc)
            s10auc.append(s0auc)
            r10auc.append(r0auc)
        ...
        bf1 = f1_score(bpredicted_labels[:lenth], y_test[0],average='micro')
        kf1 = f1_score(kpredicted_labels[:lenth], y_test[0],average='micro')
        sf1 = f1_score(spredicted_labels[:lenth], y_test[0],average='micro')
        rf1 = f1_score(rpredicted_labels[:lenth], y_test[0],average='micro')
        ...
        s0f1.append(sf1)
        r0f1.append(rf1)
        bauc.append(np.mean(b10auc))
        kauc.append(np.mean(k10auc))
        sauc.append(np.mean(s10auc))
        rauc.append(np.mean(r10auc))
    ...
    f1result = {'b': bnacc, 'k': knacc, 's': snacc, 'r': rnacc, }
    aucresult = {'b': bnacc, 'k': knacc, 's': snacc, 'r': rnacc, }
    file_name1 = r"accuracy/" + dataname + '_' + method + "_all_accuracy.mat"
    file_name2 = r"accuracy/" + dataname + '_' + method + "_all_f1.mat"
    file_name3 = r"accuracy/" + dataname + '_' + method + "_all_auc.mat"
    scio.savemat(file_name1, accresult)
    scio.savemat(file_name2, f1result)
    scio.savemat(file_name3, aucresult)
    #
