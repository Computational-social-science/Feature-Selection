...
#             'Prostate_GE', 'TOX_171', 'Orlraws10P', 'CLL_SUB_111','Yeoh','Musk1', 'Sorlie', 'Yale', 'WarpAR10P', 'GLIOMA', 'Arcene',]# 'CLL_SUB_111','Yeoh','Arcene','Orlraws10P', 'Prostate_GE', 'TOX_171', 'Su',  'WarpPIE10P',
datasets = ['Colon','SRBCT']
...
count = pd.DataFrame(index=range(len(datasets)), columns=range(len(methodsname)))
...
for dataname in datasets:
    ...
    for nowy in Class:
        ...
        print(ifdep.call_count)
        count['EAMBsp'][dataname] = int(ifdep.call_count)
        print(count)
        count.to_excel('../tables/' + 'CIcountsp.xlsx', index=True)
        break
    ...
    #     break
            ...
            #     key: [int(x) for x in value] for key, value in result_dict.items()
            # }
            # result_dict_fixed = {
            ...
            # # 保存为 JSON 文件
            # file_name = r"../feature_select_result/EAMB" +'/' + dataname + "_sp_result" + str(fold) + ".txt"
