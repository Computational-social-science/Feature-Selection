    ...
    while oldCMB != CMB and not CF==[]:
        ...
        if oldCMB != CMB or first :
            ...
            if worst in CMB:
                ...
                nowmb = CMB.copy()
            # print(features,worst,target,nowmb)
            if worst and ifdep(features,worst,target,nowmb):
                CMB.remove(worst)
...
def compare_compute_dep(old_compute_dep, new_compute_dep, data, n_states, n_cases, num_tests=100, tol=1e-6):
    ...
    return diffs
#'Isolet','EAMB',EAMB,
methodsname = ['EAMB']#,,,,'FBED','LRH''IAMB','GSMB','FBED','LRH','IAMB',,'Fast_IAMB'Inter_IAMB',',
methods = [EAMB]#,,,,FBEDLRHIAMB,,GSMB,FBED,LRH,IAMB,Fast_IAMB,Inter_IAMB,
...
# datasets = [ 'Arcene','Authorship','CLL_SUB_111','Colon','Dermatology','dexter','dna', 'Factors','GLIOMA','Isolet','madelon','Movement_libras','Musk1','ORL','Orlraws10P','Pixel',  'Prostate_GE', 'Sorlie', 'Su','SRBCT','spambase','splice','Synthetic_control', 'TOX_171', 'Waveform','Wdbc',  'WarpPIE10P','WarpAR10P', 'Yeoh', 'Yale', ]#"pigs"
