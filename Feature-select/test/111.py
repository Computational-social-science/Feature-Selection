...
# datasets = []
datasets = [ 'Pixel','Factors', 'GLIOMA', 'Su', 'Isolet','ORL', 'WarpPIE10P',
            'Prostate_GE', 'TOX_171', 'Orlraws10P', 'madelon', 'CLL_SUB_111','Yeoh', 'Yale', 'WarpAR10P', 'Arcene',]#
#
# base_url = r"./dataset/"
# import numpy as np
#
...
for dataname in datasets:
    ...
    for fold, (train_index, test_index) in enumerate(kfold.split(data_processed)):  # fold赋值之后就是0-9
        ...
        allresult = {}
        for nowy in Class:
            result = LRH_sp(data_train, nowy)
            allresult[nowy] = result
            # print(allresult)
            file_name = r"FeatureSelect/" + 'LRH-sp' + '/' + dataname + "_result" + str(fold) + ".txt"
            # np.savetxt(file_name, allresult, fmt='%d')  # 使用savetxt()函数保存数组到文件
            #保存到txt文件
            import json
            result_dict_serializable = {
                key: [int(x) for x in value] for key, value in allresult.items()
            }
            result_dict_fixed = {
