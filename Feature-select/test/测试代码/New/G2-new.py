...
from csmdccmr import cmiddspecial
...
# # mb_write_in_file_path = 'mb_results.txt'  # File path to write results
# datanames = [  'Synthetic_control','Authorship','Factors','Musk1','Su','Sorlie', 'Movement_libras','Yeoh','TOX_171', 'Orlraws10P', 'CLL_SUB_111', 'Yale', 'WarpAR10P', 'GLIOMA',   'ORL', 'WarpPIE10P','Pixel','Waveform',]#,],,
# #  #'Wdbc','Isolet',
datanames = [ 'Waveform','Wdbc', 'Synthetic_control','Authorship', 'Factors', 'Pixel',
            'Prostate_GE', 'Musk1', ]#
...
def compute_dep(var, target, cond):
    ...
    return dep
def compute_dep_s(var, target, cond, target_value):
    n_cond_states = 1
    for i in range(max_max_cond_size):#对于每个条件子集中的元素
        if cond[i] != -1:
            n_cond_states *= n_states[cond[i]]#n_cond_states就是所有种可能的次数
    ...
    # 初始化频数表
    ss_cond = np.zeros(n_cond_states, dtype=int)#记录每种情况出现的次数
    ss_target = np.zeros((n_cond_states, n_states[target]), dtype=int)#记录每种情况出现时，目标变量对应状态出现的次数，比如a情况，目标b的不同取值分别取了几次
    ss_var = np.zeros((n_cond_states, n_states[var]), dtype=int)##记录每种情况出现时，所求变量对应状态出现的次数，比如a情况，所求c的不同取值分别取了几次
    ss = np.zeros((n_cond_states, n_states[target], n_states[var]), dtype=int)##记录每种情况出现时，目标变量对应状态出现的次数，三者都记录

    # 填充频数表
    for i in range(n_cases):
        cond_state = 0#对于每个样本都是从零开始
        for j in range(max_max_cond_size):#对于每一种条件子集中的元素
            if cond[j] != -1:
                #状态编号，不同的节点用不同的状态，一维数组记录，这里是计算当前样例的状态
                #当前状态=当前+
                cond_state = cond_state * n_states[cond[j]] + data_processed[cond[j]][i]#顺序编号
    ...
    # # 计算 G² 统计量
    # statistic = np.zeros(n_states[target])
    # for i in range(n_cond_states):
    #     if ss_cond[i] > 0:#如果这种情况出现了
    #         for j in range(n_states[target]):#对于这种情况的每一种取值：
    #             if ss_target[i][j] > 0:
    #                 for k in range(n_states[var]):
    #                     if ss[i][j][k] > 0:
    #                         expected = (ss_target[i][j] * ss_var[i][k]) / ss_cond[i]
    #                         statistic[j] += ss[i][j][k] * (np.log(ss[i][j][k]) - np.log(expected))

    # 计算 G² 统计量，仅计算 target 值为 5 时
    statistic = 0.0
    for i in range(n_cond_states):
        if ss_cond[i] > 0:
            # target_value = 5  # 指定 target 值为 5
            if target_value < n_states[target] and ss_target[i][target_value] > 0:
                for k in range(n_states[var]):
                    if ss[i][target_value][k] > 0:
                        expected = (ss_target[i][target_value] * ss_var[i][k]) / ss_cond[i]
                        statistic += ss[i][target_value][k] * (np.log(ss[i][target_value][k]) - np.log(expected))
...
def Inter_IAMB(targets):
    ...
    for target_value in targets:
        # print(target,n_states[target])
        # for target_value in range(0, n_states[target]):
        target = n_vars-1
        ...
        while True:
            ...
            if n_conds < max_max_cond_size:
                ...
                for i in range(n_vars):
                    if mb[i] == 0:
                        dep[i] = compute_dep_s(i, target, cond, target_value)
                        if dep[i] >= 1.0:
                            stop = False
            if not stop:
                aux = k_greedy(dep)
