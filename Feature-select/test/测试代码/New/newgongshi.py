...
from 特征选择过程 import new_gongshi
from 特征选择过程.new_gongshi import csmdccmrn

# 忽略特定类型的警告
warnings.filterwarnings("ignore", message="A column-vector y was passed when a 1d array was expected.*")
warnings.filterwarnings("ignore", category=pd.errors.PerformanceWarning)
# ====================================数据导入=====================================
datasets = ['Sorlie', 'Dermatology','Musk1','Synthetic_control','Waveform','Wdbc','Authorship', ]#
# datasets = [ ,'Movement_libras', 'Su','Factors', 'Pixel', 'Isolet', 'Yale', 'Yeoh', 'WarpAR10P', 'GLIOMA', 'Arcene', 'ORL', 'WarpPIE10P',
...
#             #     cpick = csmdccmrn(X_train.values, np.ravel(y_train), ck, n_selected_features=time)
#             #     result.append(cpick)
...
#处理得到结果计算出的F1和AUC
T = '1'#用于区别储存
methods = ['csmdccmrn',]#'cife','mrmr','jmim', 'mri', 'cfr', 'dcsf', 'mrmd', 'iwfs', 'ucrfs',
    ...
    for dataname in datasets:#对于每个数据集进行处理
        ...
        for fold, (train_index, test_index) in enumerate(kfold.split(y)):  # fold赋值之后就是0-9
            ...
            for i in range(0, feature_num):  # 对不同特征数量的结果进行处理
                ...
                time_bf1 = f1_score(bpredict_labels[i], y_test[0], average='micro')
                time_kf1 = f1_score(kpredict_labels[i], y_test[0], average='micro')
                time_sf1 = f1_score(spredict_labels[i], y_test[0], average='micro')
                time_rf1 = f1_score(rpredict_labels[i], y_test[0], average='micro')
                time_bacc = calculate_accuracy(bpredict_labels[i], y_test[0])
                time_kacc = calculate_accuracy(kpredict_labels[i], y_test[0])
                time_sacc = calculate_accuracy(spredict_labels[i], y_test[0])
                time_racc = calculate_accuracy(rpredict_labels[i], y_test[0])
                print(time_bacc)
                ...
                fold_b_acc.append(time_bacc)
                fold_k_acc.append(time_kacc)
                fold_s_acc.append(time_sacc)
                fold_r_acc.append(time_racc)
            ...
            allfold_b_acc.append(fold_b_acc)
            allfold_k_acc.append(fold_k_acc)
            allfold_s_acc.append(fold_s_acc)
            allfold_r_acc.append(fold_r_acc)
        ...
        kf1table[method][iposition] =  np.mean(allfold_k_f1 )
        sf1table[method][iposition] =  np.mean(allfold_s_f1 )
        rf1table[method][iposition] =  np.mean(allfold_r_f1 )
        bacctable[method][iposition] = np.mean(allfold_b_acc)
        kacctable[method][iposition] = np.mean(allfold_k_acc)
        sacctable[method][iposition] = np.mean(allfold_s_acc)
        racctable[method][iposition] = np.mean(allfold_r_acc)
...
tables = [bf1table, sf1table, kf1table, rf1table, bacctable, sacctable, kacctable, racctable]#,bauctable, sauctable, kauctable, rauctable
# 计算每列的平均值
...
#存储
bf1table.to_excel( 'tables/'+T+'/new_ten-bf1table.xlsx', index=True)
sf1table.to_excel( 'tables/'+T+'/new_ten-sf1table.xlsx', index=True)
kf1table.to_excel( 'tables/'+T+'/new_ten-kf1table.xlsx', index=True)
rf1table.to_excel( 'tables/'+T+'/new_ten-rf1table.xlsx', index=True)
...
bacctable.to_excel('tables/'+T+'/new_ten-bacctable.xlsx', index=True)
sacctable.to_excel('tables/'+T+'/new_ten-sacctable.xlsx', index=True)
kacctable.to_excel('tables/'+T+'/new_ten-kacctable.xlsx', index=True)
racctable.to_excel('tables/'+T+'/new_ten-racctable.xlsx', index=True)
#绘制箱线图
# import matplotlib.pyplot as plt
# import numpy as np
