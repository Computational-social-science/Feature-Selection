        ...
        for fold, (train_index, test_index) in enumerate(kfold.split(y)):  # fold赋值之后就是0-9
            ...
            for i in range(0, feature_num):  # 对不同特征数量的结果进行处理
                ...
                if method in ['csmdccmr', 'csmdccmr2']:
                    for j in range(0, len(classes)):#不同种类分类器
                        bauc = roc_auc_score(y_true_binarized, bpredict_probabity[i][j], average='macro', multi_class='ovr')
                        kauc = roc_auc_score(y_true_binarized, kpredict_probabity[i][j], average='macro', multi_class='ovr')
                        sauc = roc_auc_score(y_true_binarized, spredict_probabity[i][j], average='macro', multi_class='ovr')
                        rauc = roc_auc_score(y_true_binarized, rpredict_probabity[i][j], average='macro', multi_class='ovr')
            ...
            fold_b_auc.append(all_auc_b)
            fold_k_auc.append(all_auc_k)
            fold_s_auc.append(all_auc_s)
            fold_r_auc.append(all_auc_r)
